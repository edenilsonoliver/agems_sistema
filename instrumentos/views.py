...
def arquivo_upload(request, instrumento_id):
    ...
    try:
        ...
        if mime_type not in ALLOWED_MIMES:
            # Algumas instalações do magic retornam application/octet-stream para arquivos Office
            if not (ext in ['.docx', '.xlsx'] and mime_type in ['application/octet-stream', 'application/zip']):
    ...
    # Verificação de Macros (para arquivos Office)
    if ext in ['.docx', '.xlsx']:
        try:
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': 'Erro ao salvar no banco de dados.'})
