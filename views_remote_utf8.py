...
# Endpoint AJAX para obriga├º├Áes (usado na cria├º├úo de A├º├Áes)
...
class AcaoListView(PermissionRequiredMixin, ModernListView):
    """
    Lista as A├º├Áes (n├¡vel de execu├º├úo vinculado ├á Obriga├º├úo).
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        # OTIMIZA├ç├âO: Se filtrar obriga├º├Áes para o dropdown, carregue apenas o necess├írio
...
class AcaoCreateView(PermissionRequiredMixin, ModernCreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Contexto para Fiscaliza├º├úo (Fase 5)
        # Passar IDs dos tipos que s├úo "Fiscaliza├º├úo" para l├│gica no frontend
        context['fiscalizacao_ids'] = list(TipoAcao.objects.filter(
            nome__icontains='Fiscaliza├º├úo'
        ).values_list('id', flat=True))
        
        return context

    def save_assets(self, acao, docs_formset, fotos_formset):
        """Helper para salvar ativos na cria├º├úo"""
        try:
            # Documentos
    ...
    def form_valid(self, form):
        ...
        if is_valid:
            # Validar checklist obrigat├│rio (m├¡nimo 1 item v├ílido)
            itens_validos = 0
            ...
            if itens_validos == 0:
                messages.error(
                    self.request,
                    '├ë obrigat├│rio adicionar pelo menos 1 item no checklist de tarefas operativas.'
                )
            ...
            messages.success(self.request, f'A├º├úo "{self.object.nome}" criada com sucesso!')
            return super().form_valid(form)
        else:
            # Tratamento de Erros Amig├ível (UX)
            ...
            for form_erro in docs_formset.errors:
                if form_erro:
                    for campo, msgs in form_erro.items():
                         for msg in msgs:
                             # Se o erro for no campo arquivo, mostra mensagem espec├¡fica
            ...
            if not docs_formset.errors and not fotos_formset.errors:
                 messages.error(self.request, "Verifique os dados do formul├írio.")
...
class AcaoUpdateView(PermissionRequiredMixin, ModernUpdateView):
    ...
    def save_assets(self, acao, docs_formset, fotos_formset):
        """Helper para salvar documentos e fotos com inje├º├úo de usu├írio"""
        try:
            # Documentos
            docs = docs_formset.save(commit=False)
            for doc in docs:
    ...
    def form_valid(self, form):
        ...
        if is_valid:
            ...
            messages.success(self.request, f'A├º├úo "{self.object.nome}" atualizada com sucesso!')
            return super().form_valid(form)
        else:
            # Tratamento de Erros Amig├ível (UX) - UpdateView
...
def acoes_json(request):
    """Retorna as a├º├Áes em formato JSON para o FullCalendar"""
    user = request.user
...
def listar_marcadores_ajax(request, acao_id):
    """Lista todos os marcadores de uma a├º├úo espec├¡fica"""
...
def salvar_marcador_ajax(request, acao_id):
    ...
    try:
        ...
        if not titulo or not latitude or not longitude:
            return JsonResponse({
                'status': 'error', 
                'message': 'T├¡tulo, Latitude e Longitude s├úo obrigat├│rios.'
            }, status=400)
        ...
        # Processar novas fotos espec├¡ficas do marcador se enviadas
        fotos_count = 0
...
def deletar_marcador_ajax(request, marcador_id):
    ...
    # Fotos vinculadas n├úo s├úo deletadas, apenas perdem o v├¡nculo com o marcador
    # Isso garante que a evid├¬ncia de campo continue na aba Fotos.
    marcador.delete()
